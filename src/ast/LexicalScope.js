...
class LexicalScope {
  constructor(parent, others) {
    this.parent = parent // parent of scope
    this.others = others 
    this.id = scopeId++ // scope id
    this.table = {} // scope varialbe table
    this.children = [] // children of scope 
    this.index = 0 // varialbe number of this scope
    if(!this.parent) {
      this.globalHash = {}
    } else {
  ...
  getLexemeName(id) {
    ...
    if(scope) {
      ...
      return `syntax error: lexeme ${id} not found`
    }
  }
  ...
  print(level = 0) {
    ...
    for(let key in this.table) {
      console.log(`${pad}  ${key} : ${this.table[key].value}`)
    }
